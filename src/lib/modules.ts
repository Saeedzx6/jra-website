...
// Typed as a non-empty tuple, not a plain array: the membership page reads
// [0] for its hero lede, and under noUncheckedIndexedAccess a plain array
// makes that `undefined`. The list is a fixed editorial constant that always
// has a first entry, so the type says so rather than the callers guarding.
export const membershipTypes: [Item & { cta: Localized }, ...Array<Item & { cta: Localized }>] = [
  {
...
// Non-empty tuple for the same reason as membershipTypes above — the training
// page reads [0] for its hero lede.
