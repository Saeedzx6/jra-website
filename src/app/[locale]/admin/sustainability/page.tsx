...
export default async function AdminSustainabilityPage() {
  ...
  return (
    <div>
      <h1 className={ui.sectionTitle}>{tn("sustainabilityBenchmarks")}</h1>
      ...
      <form action={upsertBenchmark} className="mt-6 flex flex-wrap gap-2 rounded-2xl border border-rule bg-surface p-4">
        <input name="metricKey" placeholder={ts("metricKeyPlaceholder")} required className="flex-1 min-w-[180px] rounded-lg border border-rule bg-paper px-3 py-2 text-sm" />
        <input name="sectorAvg" type="number" step="any" placeholder={ts("sectorAveragePlaceholder")} required className="w-40 rounded-lg border border-rule bg-paper px-3 py-2 text-sm" />
        <input name="unit" placeholder={ts("unitPlaceholder")} required className="w-28 rounded-lg border border-rule bg-paper px-3 py-2 text-sm" />
        ...
        {benchmarks.map((b) => (
          <div key={b.metricKey} className="flex items-center justify-between rounded-xl border border-rule bg-surface p-4">
            ...
            <form action={deleteBenchmark.bind(null, b.metricKey)}>
              <button className="text-xs text-accent hover:underline">{ta("remove")}</button>
            </form>
          </div>
        ))}
