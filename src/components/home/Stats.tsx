...
import { stats, pick } from "@/lib/content";
 ...
 * and it stays correct if the user changes the preference while the page is open.
 * The server snapshot is `false` so SSR and first client render agree.
 */
      ...
      {stats.map((stat) => (
        <li key={stat.label.en}>
          <Counter
            ...
            reduced={reduced}
          />
          <span className="label">{pick(stat.label, locale)}</span>
        </li>
      ))}
    </ul>
  );
...
function Counter({
   ...
   * and renders ٧١٨, while every other number on the site goes through
   * next-intl and renders 718. One page showing both is the kind of detail
   * that makes a bilingual build look unfinished. One formatter, one
   * numbering system.
   */
