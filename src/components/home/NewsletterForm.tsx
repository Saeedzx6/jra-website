...
export function NewsletterForm() {
  ...
  function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    // Interests are stored in English regardless of the reading locale — they
    // are a stable key for segmenting the mailing list, not display text.
    for (const interest of interests) formData.append("interests", interest);

    startTransition(async () => {
      const result = await subscribeToNewsletter({ ok: false }, formData);
      if (result.ok) {
        setDone(true);
      } else {
        setError(result.error === "rate_limited" ? t("rateLimited") : t("serverError"));
      }
    });
  }
